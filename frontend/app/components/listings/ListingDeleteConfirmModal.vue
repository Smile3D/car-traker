...
<template>
  <BaseModal :title="t('listingDetails.deleteConfirmTitle')" @close="emit('cancel')">
    <div class="space-y-4">
      <div class="flex items-start gap-3">
        <ExclamationTriangleIcon class="mt-0.5 size-5 shrink-0 text-destructive" />
        <p class="text-sm text-foreground">{{ t('listingDetails.deleteConfirmMessage') }}</p>
      </div>

      <div>
        <label class="block text-sm font-medium text-foreground" for="listing-delete-reason">
          {{ t('listingDetails.deleteReasonLabel') }}
        </label>
        <textarea
          id="listing-delete-reason"
          v-model="reason"
          rows="3"
          :placeholder="t('listingDetails.deleteReasonPlaceholder')"
